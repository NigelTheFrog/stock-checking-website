    <script>
        ...
        function openModalDetailCSO(itemName, dataItem) {
            ...
            $('#ModalDetailCsoBatch').modal('show');
        }

        function csoUlang() {
            const itemIdParam = document.getElementById("itembatchid").value; // Get the selected gudang values
                ...
                success: function(data) {
                    ...
                    if (data['result'] == 1) {
                        Swal.fire({
                            icon: "success",
                            title: "Berhasil",
                            text: `Batch Item dengan id ${itemIdParam}\nberhasil di CSO Ulang`,
                        });
        ...
        function openModalCsoUlang() {
            $('#ModalDetailCsoBatch').modal('hide');
        ...
        function hapusItemTemuan(itembatchid, trsdetid) {
            $.ajax({
                url: "{{ url('admin/dashboard/avalan/hapus-temuan-avalan') }}",
                method: "POST",
                data: {
                ...
                success: function(data) {
                    if (data['result'] == 1) {
                        Swal.fire({
                            icon: "success",
                            title: "Berhasil",
                            text: `Temuan batch item dengan item batch id ${itembatchid}\nberhasil dihapus`,
                        });
                    } else {
                        Swal.fire({
                            icon: "error",
        ...
        function sortDetailCSO(order, columnIndex, iconSort) {
            const table = document.getElementById('tabelDetailBatch');
            // console.log(table);
            const rows = Array.from(table.rows).slice(1); // Exclude the header row
            const isNumeric = !isNaN(rows[0].cells[columnIndex].textContent.trim());
            const allIconSortItem = document.getElementsByClassName('sortDetailBatchIcon');
            ...
            resetRowNumbers('tabelDetailBatch',0);
        }
    </script>
