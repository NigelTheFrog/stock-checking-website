    ...
    <div class="mb-3">
        @if ($checkItemType->statusitem == 'TR')
            <button type="button" class="btn btn-danger "
                onclick="hapusItemTemuan('{{ $itemid }}', '{{ $trsdetid }}')"><i class="fas fa-trash-alt"></i>
                Hapus Item</button>
        @endif
    </div>
@endif
...
<div style="max-height: 15vh;overflow-y: auto; margin-bottom: 2vh;">
    <table class="table table-sm table-bordered  small mb-3 text-center" style=" table-layout: auto;">
        <thead>
            <tr>
                <th class="align-middle" style="background-color:	#E5E4E2;">Warehouse</th>
                <th class="align-middle" style="background-color:	#E5E4E2;">Qty</th>
                <th class="align-middle" style="background-color:	#E5E4E2;">Status</th>
                <th class="align-middle" style="background-color:	#E5E4E2;">Total CSO</th>
                <th class="align-middle" style="background-color:	#E5E4E2;">Selisih</th>
        ...
        <tbody>
            ...
            @foreach ($itemWrh as $wrhItem)
                <tr @if ($wrhItem->qtyCso != $wrhItem->qtyWrh && $wrhItem->qtyCso != null) class="table-danger" @endif>

                    <td class="align-middle" style='white-space:nowrap;'>{{ $wrhItem->wrh }}</td>
                    <td class="align-middle" style='white-space:nowrap;'>{{ $wrhItem->qtyWrh }}</td>
                    <td class="align-middle" style='white-space:nowrap;'>
                        @if ($wrhItem->qtyCso == null)
                            <span class='badge rounded-pill text-bg-warning text-wrap' style='width: 5rem'>Belum
                                Proses </span>
                        @elseif ($wrhItem->qtyCso > $wrhItem->qtyWrh)
                    ...
                    <td class="align-middle" style='white-space:nowrap;'>{{ $wrhItem->qtyCso - $wrhItem->qtyWrh }}</td>
        ...
        <tbody>
            @foreach ($tableDetailDashboard as $detail)
                ...
                <tr>
                    ...
                    <td>{{ $detail->grade }} @if ($detail->description != null)
                            ({{ $detail->description }})
                        @endif
                    </td>
                    <td>{{ $detail->remark }}</td>
...
<div class="row mb-2">
    ...
    <div class="col-2">
        ...
        <div class="form-check">
            <input class="form-check-input" type="checkbox" id="checkkesalahanadmin" name="check_kesalahan_admin"
                @if ($dataAdminBatch->kesalahan_admin == 1) checked @endif @if (Auth::user()->level != 1 && Auth::user()->level != 2 && Auth::user()->level != 3) disabled @endif>
        ...
        <div class="form-check">
            <input class="form-check-input" type="checkbox" id="checkbatchTertukar" name="check_batch_tertukar"
                @if ($dataAdminBatch->batch_tertukar == 1) checked @endif @if (Auth::user()->level != 1 && Auth::user()->level != 2 && Auth::user()->level != 3) disabled @endif>
        ...
        <div class="form-check"> {{-- // tidak di hitung --}}
            <input class="form-check-input" type="checkbox" id="checkTidakDiHitung" name="check_tidak_hitung"
                @if ($dataAdminBatch->tidak_hitung == 1) checked @endif @if (Auth::user()->level != 1 && Auth::user()->level != 2 && Auth::user()->level != 3) disabled @endif>
    ...
    <div class="col-3 small">
        ...
        <div class="w-100 mb-2">
            ...
            <input type="number" class="form-control form-control-sm" name="groupValue" id='groupValue' disabled
                value=''>
        </div>
    </div>
</div>
