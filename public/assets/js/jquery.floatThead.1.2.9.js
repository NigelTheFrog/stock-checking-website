...
!(function (a) {
    ...
    var f = window._,
      ...
      i = function () {
        if (g) return !1
        var b = a(
          "<table><colgroup><col></colgroup><tbody><tr><td style='width:10px'></td></tbody></table>",
        )
        a("body").append(b)
    ...
    a.fn.floatThead = function (l) {
      ...
      return (
        a.each(l, function (b) {
          b in a.floatThead.defaults ||
            !o.debug ||
            c(
              "jQuery.floatThead: used [" +
                b +
        ...
        this.filter(":not(." + o.floatTableClass + ")").each(function () {
          ...
          function m() {
            ;(C =
              (f.isFunction(o.scrollingTop)
                ? o.scrollingTop(z)
                : o.scrollingTop) || 0),
              (D =
                (f.isFunction(o.scrollingBottom)
                  ? o.scrollingBottom(z)
                  : o.scrollingBottom) || 0)
          }
          function n() {
            ...
            if (V) b = U.find("col").length
              ...
              var d
              ;(d =
                null == o.cellTag && o.headerCellSelector
                  ? o.headerCellSelector
                  : "tr:first>" + o.cellTag),
                (c = A.find(d)),
                (b = 0),
                c.each(function () {
            ...
            if (b != H) {
              ...
              for (var e = [], f = [], g = [], i = 0; b > i; i++)
                e.push('<th class="floatThead-col"/>'),
                  f.push("<col/>"),
                  g.push(
              ...
              ;(f = f.join("")),
                (e = e.join("")),
                h && ((g = g.join("")), W.html(g), (bb = W.find("fthtd"))),
                Z.html(e),
                ($ = Z.find("th")),
                V || U.html(f),
                (_ = U.find("col")),
          ...
          function w() {
            ...
            return function (d, e, f) {
              null == d ||
                (a == d.top && b == d.left) ||
                (X.css({ top: d.top, left: d.left }), (a = d.top), (b = d.left)),
                e && l(),
                f && i()
              var g = I.scrollLeft()
          ...
          function x() {
            if (I.length) {
              var a = I.width(),
                ...
                e = d > a ? G : 0,
                f = c > b ? G : 0
              ;(F.horizontal = d > a - f ? G : 0),
                (F.vertical = c > b - e ? G : 0)
            }
          }
          var y = k,
            z = a(this)
          ...
          if (
            (Y.append(Z),
            z.prepend(U),
            h && (N.append(W), z.append(N)),
            S.append(T),
            X.append(S),
            o.copyTableClass && S.attr("class", z.attr("class")),
            S.attr({
            ...
            S.addClass(o.floatTableClass).css("margin", 0),
            J)
          ) {
            var cb = function (a, b) {
                ...
                X.css(e),
                  (a = a
                    .wrap(
                      "<div class='" +
                        o.floatWrapperClass +
                        "' style='position: relative; clear:both;'></div>",
                    )
                    .parent()),
                  (P = !0)
              }
          ...
          X.css({
            ...
            X.addClass(o.floatContainerClass),
            m()
          var db = { "table-layout": "fixed" },
            eb = { "table-layout": z.css("tableLayout") || "auto" },
            fb = z[0].style.width || ""
          x()
          var gb,
            hb = function () {
          ...
          O
            ? J
              ? I.on(c("scroll"), mb)
              : (I.on(c("scroll"), mb), j.on(c("scroll"), lb))
            : j.on(c("scroll"), lb),
            j.on(c("load"), ob),
            b(o.debounceResizeMs, c("resize"), nb),
            z.on("reflow", ob),
            e(z) && z.on("filter", ob).on("sort", ob).on("page", ob),
            z.data("floatThead-attached", {
              destroy: function () {
                var a = ".fth-" + y
                q(),
                  z.css(eb),
                  U.remove(),
                  h && N.remove(),
                  Y.parent().length && Y.replaceWith(A),
                  z.off("reflow"),
                  I.off(a),
                  P && (I.length ? I.unwrap() : z.unwrap()),
                  J && z.css("minWidth", ""),
                  X.remove(),
                  z.data("floatThead-attached", !1),
                  j.off(a)
              },
              reflow: function () {
                ob()
              ...
              getRowGroups: function () {
                return E
                  ? X.find("thead").add(z.find("tbody,tfoot"))
                  : z.find("thead,tbody,tfoot")
              },
            }),
            k++
        }),
