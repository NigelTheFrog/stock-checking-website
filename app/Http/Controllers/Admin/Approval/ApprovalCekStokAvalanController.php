...
class ApprovalCekStokAvalanController extends Controller
{
    /**
    ...
    {
        $getDbtTrsHed = DB::table('dbttrsheda')->orderByDesc('createddate')->whereNot('statusdoc', '=', 'A')->get();
        $dataApproval = User::select('id', 'name')
            ...
            ->orWhere('level', '10')
            ->get();
        return view('admin.approval.cek-stok-avalan', ['listNodoc' => $getDbtTrsHed, "dataApproval" => $dataApproval]);
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route("approval-avalan.index")->with('status', "$message berhasil melakukan approval");
    }
