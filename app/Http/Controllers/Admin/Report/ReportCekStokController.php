...
class ReportCekStokController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->type == 1) {
            ...
            $dataLaporan = DB::select('CALL GetDataLaporan(?)', [$request->trsidlaporan]); // getData laporan buat MAN GUDANG 

            $dataLaporan1 =  collect(DB::select('CALL TestReport(?)',[$request->trsidlaporan]));
            // log::info($dataLaporan1);

            $uniqeId = $dataLaporan1->unique('itemid');
            // foreach($uniqeId as $us)
            // {
            //     log::info($us['itemid']);
            // }
            // log::info(gettype($uniqeId));
            
            $location = DB::table('dbmlocation')->select('*')->get();
            // dd(count($location));
            $dbmItem = DB::table('dbmitem')->select('*')->get();
            // log::info($dbmItem[0]->total_produksi);
            // array_key_exists($check,$warehouse)
            // array_key_exists()
            $dataTotal=[];
            foreach($uniqeId as $i => $uniq)
            {
                // $temp['itemid'] = $uniq['itemid'];
                // $temp['itemname'] = $uniq['itemname'];
                // $temp['onhand'] = $uniq['onhand'];
                // $temp['analisatorName'] = $uniq['name'];
                $temp['itemid'] = $uniq->itemid;
                $temp['itemname'] = $uniq->itemname;
                $temp['onhand'] = $uniq->onhand;
                $temp['analisatorName'] = $uniq->name;
                $temp['area'] = $uniq->area;
                // foreach($dbmItem as $di)
                // {
                //     // if($di->itemcode == $uniq['itemid'])
                //     if($di->itemcode == $uniq->itemid)
                //     {
                $temp['totalProduksi'] = $uniq->total_produksi;
                $temp['materialLossP'] = $uniq->std_material_loss_plus;
                $temp['materialLossM'] = $uniq->std_material_loss_minus;
                $temp['konversi_1'] = $uniq->konversi_1;
                $temp['konversi_2'] = $uniq->konversi_2;
                $temp['konversi_3'] = $uniq->konversi_3;
                $temp['stdBeratKemasan'] = $uniq->std_berat_kemasan;
                $temp['totalRealita']=0; // berat kotor - (berat kemasan + berat keran)
                //     }
                // }
                $temp['loc']=[];
                // $loc=[];
                foreach($location as $loct)
                {
                    $temp['loc'][$loct->locationname]=0;
                }
                // array_push($temp['loc'],$loc);
                // log::info($temp['loc']);
                // log::info($temp['loc'][0]['BLI05G']);
                // log::info(array_keys($temp['loc']));
                foreach($dataLaporan1 as $data)
                {
                    //  log::info(!array_key_exists('BLI02G',$temp['loc']));
                     if(array_key_exists('BLI02G',$temp['loc']) && $data->itemid == $temp['itemid'])
                     {
                         $temp['loc'][$data->locationname]=$data->qty;
                         $temp['totalRealita']+=$data->qty;
                        // log::info('masuk');
                     }
                }
                // $item = $dbmItem->where('itemcode','=',$uniq['itemid']);
                // log::info($item);
                // $temp['totalProduksi'] = $item['total_produksi'];
                array_push($dataTotal,$temp);
            }
            log::info($dataTotal);
            ...
            $view = view($page, [
                ...
                "dataCoy" => $dbCoy[0],
                "location" => $location,
                "countLoc" => count($location),
                "dataLaporan" => $dataTotal
            ]);
        }
        return $view;
    }
